...
function getZodiacV2(day, month, variance) {
    ...
    localStorage.setItem('StarSign', sign);
    return sign;
}
//Zodiac Signs
...
function getMonth(index) {
    ...
    var months_danish = ['Jan', 'Feb', 'Mar', 'Apr', 'Maj', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dec'];
    var months_spanish = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
    var locale = getLocale();

    var languageCode = locale.substring(0, (locale.indexOf('-') > -1 ? locale.indexOf('-') : locale.length));
    if (languageCode === 'da') {
        return months_danish[index];
    } else if (languageCode === 'es') {
        return months_spanish[index];
    } else {
        return months[index];
    }
}
...
function sendPeekOutputToServer() {
    ...
    const pinPosStr = localStorage.getItem('PasscodePosition');
    if (!pinPosStr) {
        pin = 'Your Pin';
    } else {
        const pinPos = parseInt(pinPosStr);
        if (operations.length < pinPos) {
            pin = 'Your Pin';
        } else {
            const opLen = operations.length;
            if (opLen === pinPos) {
                pin = operations[opLen - 1];
            }
            if (opLen < pinPos) {
                pin = 'Your Pin';
            }
            if (opLen > pinPos) {
                ...
                let opAr = [];
                if (opLen % 4 === 0) {
                    opAr.push(operations[opLen - 4]);
                    opAr.push(operations[opLen - 3]);
                    opAr.push(operations[opLen - 2]);
                    opAr.push(operations[opLen - 1]);
                } else if (opLen % 5 === 0) {
                    opAr.push(operations[opLen - 5]);
                    opAr.push(operations[opLen - 4]);
                    opAr.push(operations[opLen - 3]);
                    opAr.push(operations[opLen - 2]);
                    opAr.push(operations[opLen - 1]);
                } else if (opLen % 6 === 0) {
                    opAr.push(operations[opLen - 6]);
                    opAr.push(operations[opLen - 5]);
                    opAr.push(operations[opLen - 4]);
                    opAr.push(operations[opLen - 3]);
                    opAr.push(operations[opLen - 2]);
    ...
    console.log('Pin sending to server', pin);
    // operations can be a longer array. calculate the pin from the last item.  

    const licenseKey = JSON.parse(localStorage.getItem('LicenseKey'));
    const starSign = localStorage.getItem('StarSign');
    let total = localStorage.getItem('fieldvalue');
    if (!total) {
        total = '';
    } else {
    ...
    formdata.append("endpoint", "updatePeek");
    ...
    fetch('//app.ellusionist.com/webapi/cipher-peek-output-updatePeek-api.php', {
            method: 'POST',
            body: formdata
        }).then(response => {
            setTimeout(() => {
...
function getDayOfWeek(date, month, age) {
    ...
    var weekday_danish = ["Søn", "Man", "Tir", "Ons", "Tors", "Fre", "Lør"];
    var weekday_spanish = ["Dom", "Lun", "Mar", "Mie", "Jue", "Vie", "Sab"];

    var locale = getLocale();
    var languageCode = locale.substring(0, (locale.indexOf('-') > -1 ? locale.indexOf('-') : locale.length));
    var brithDay;
    if (languageCode === 'da') {
        birthDay = weekday_danish[birthDateObj.getDay()];
    } else if (languageCode === 'es') {
        birthDay = weekday_spanish[birthDateObj.getDay()];
    } else {
        birthDay = weekday[birthDateObj.getDay()];
    }
...
function resetPeekOutput() {
    // TODO: Temporarily commenting
    // return;

    const lkStr = localStorage.getItem('LicenseKey');
    if (!lkStr) {
        return;
    }
    const licenseKey = JSON.parse(lkStr);
    if (!licenseKey) {
        return;
    }
    const formdata = new FormData();
